...
def signup():
    ...
    if request.method == 'POST':
        ...
        if password != verify:
            verify_error = "Try again! Passwords do not match."
        ...
        if existing_user:
            username_error = "That member already exists"
...
def login():
    ...
    if request.method == 'POST':
        ...
        if user and user.password != password:
            password_error = "Foolish! Try again."
            return render_template('login.html', password_error=password_error)

        if not user:
            username_error = "What's your real name? ANSWER ME!!!"
